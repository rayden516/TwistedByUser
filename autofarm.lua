...
if not O then
    print("[AutoFarm] offsets failed to load")
    return
end
...
local function getStormWind(stormModel)
    if not stormModel or not stormModel.Parent then
        return 0
    end
    local wind = 0
    ...
    if windFolder then
        for _, part in ipairs(windFolder:GetChildren()) do
            ...
            if type(v) == "number" and v > wind then
                wind = v
            end
        end
    end
...
local function navSample(d)
    ...
    if n == 0 or (now - h.pts[n].t) >= 0.03 then
        ...
        while #h.pts > nav.MAX do
            table.remove(h.pts, 1)
        end
    end
...
local function pointInKillZone(pos)
    for _, c in ipairs(getDangerCircles()) do
        ...
        if dx * dx + dz * dz < c.killR * c.killR then
            return true
        end
    end
    return false
...
local function getSafeTarget(playerPos, d)
    ...
    local frontPos = Vector3.new(tx + dirX * frontOffset, torPos.Y + cfg.HeightOffset, tz + dirZ * frontOffset)
...
local function navScore(d, playerPos)
    ...
    if navBlocked(playerPos, pos) then
        score = score - NAV_W_BLOCK
    end
    return score
end
...
local function pickTarget(playerPos)
    ...
    if _curTargetName then
        for _, d in ipairs(tornadoList) do
            if d.name == _curTargetName and d.part and d.part.Parent then
                cur = d
                break
            end
        end
    ...
    if best then
        _curTargetName = best.name
        cfg.Selected = best.name
    end
    return best or cur
end
...
local function updateHud()
    ...
    local tp = getTarget()
    if tp and hrp then
        dist = string.format("%.0fm", (hrp.Position - tp.Position).Magnitude)
    end
    local cam = workspace.CurrentCamera
    if cam then
        hudLabel.Position = Vector2.new(cam.ViewportSize.X / 2, 60)
    end
    hudLabel.Text = string.format("AutoFarm | %s | mode: %s | %s | %s", _hudActivity, cfg.TargetMode, tgt, dist)
...
local function updateCompass()
    if not cfg.ShowCompass then
        hideCompass()
        return
    end

    local d = getTargetData()
    if not d or not d.part or not d.part.Parent then
        hideCompass()
        return
    end

    local cam = workspace.CurrentCamera
    if not cam or type(WorldToScreen) ~= "function" then
        hideCompass()
        return
    end
    ...
    local wind = getStormWind(d.storm)
    compassText.Position =
        Vector2.new(cx + nx * (COMPASS_RADIUS + COMPASS_LEN + 16), cy + ny * (COMPASS_RADIUS + COMPASS_LEN + 16))
        ...
        function()
            while collectActive do
                ...
                if pick then
                    ...
                    while collectActive and farmTweenLock do
                        task.wait(0.1)
                    end
                    if collectActive and isrbxactive() then
                        keypress(cfg.InteractKey)
                        local t0 = tick()
                        while collectActive and (tick() - t0) < cfg.CollectHold do
                            task.wait(0.05)
                        end
    ...
    for i = 1, cfg.ProbeCount do
        ...
        if isrbxactive() then
            ...
            repeat
                ...
                mouse1press()
                local t0 = tick()
                while cycleActive and (tick() - t0) < cfg.PlaceHold do
                    task.wait(0.05)
                end
                mouse1release()
                task.wait(0.4)
                ...
                if not rp then
                    ...
                    if not td or not (td.part and td.part.Parent) or weak then
                        ...
                        if front then
                            ...
                            while cycleActive and farmTweenLock do
                                task.wait(0.1)
                            end
                        end
                        if not cycleActive then
                        ...
                        if rp then
                            ...
                            for _, e in ipairs(tornadoList) do
                                if e.name == td.name and e.part and e.part.Parent then
                                    td = e
                                    break
                                end
                            end
                        ...
                        while cycleActive do
                            ...
                            if not frp then
                                ...
                                if dProbe < tr + 40 then
                                    reached = true
                                end
                                if reached and not pointInKillZone(Vector3.new(placedX, 0, placedZ)) then
                                    break
                                end
                                if (not reached) and dProbe > dStart + 500 then
                                    break
                                end

                                _hudActivity =
                                    string.format("Farm: %s w=%.0f", reached and "storm passing" or "waiting storm", cw)
                                navSample(cur)
                        ...
                        while cycleActive and guard <= cfg.ProbeCount + 1 and (tick() - cStart) < 45 do
                            ...
                            if e.blocked then
                                ...
                                if cycleActive and isrbxactive() then
                                    keypress(cfg.InteractKey)
                                    local t0 = tick()
                                    while cycleActive and (tick() - t0) < cfg.CollectHold do
                                        task.wait(0.05)
                                    end
                                    keyrelease(cfg.InteractKey)
                                end
                                ...
                                if crp then
                                    crp.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
                                end
                                task.wait(0.3)
                                guard = guard + 1
                            end
...
UI.AddTab(
    ...
    function(tab)
        ...
        TS:Button(
            "Mode: Nearest",
            function()
                cfg.TargetMode = "nearest"
                _curTargetName = nil
                notify("Mode: Nearest", "", 2)
            end
        )
        TS:Button(
            "Mode: Strongest",
            function()
                cfg.TargetMode = "strongest"
                _curTargetName = nil
                notify("Mode: Strongest", "", 2)
            end
        )
        TS:Button(
            "Mode: Smart",
            function()
                cfg.TargetMode = "smart"
                _curTargetName = nil
                notify("Mode: Smart", "", 2)
            end
        )
        ...
        PS:Toggle(
            "auto_cycle",
            "Auto Farm (place+collect+repeat)",
            cfg.AutoCycle,
            function(state)
                setCycle(state)
            end
        ...
        PS:Button(
            ...
            function()
                ...
                if not pr then
                    ...
                    notify("player_related not found", "Check console", 3)
                    return
                end
                ...
                if not pfold then
                    ...
                    notify("Probes folder not found", "Check console", 3)
                    return
                end
                ...
                local list = getProbesSorted()
                if #list == 0 then
                    notify("No probes found", "Check console", 3)
                    return
                end
                for i, e in ipairs(list) do
                    print(
                        string.format(
                            "[AutoFarm] #%d %s | %.0fm%s | %.0f %.0f %.0f",
                            i,
                            e.name or "?",
                            e.dist,
                            e.blocked and " [in tornado]" or "",
                            e.pos.X,
                            e.pos.Y,
                            e.pos.Z
                        )
    ...
    function()
        ...
        for i = 1, 4 do
            ...
            if kb and i <= cfg.ProbeCount then
                ...
                if cur and not probeKeyPrev[i] and not cycleActive then
                    goToProbe(i)
                end
                probeKeyPrev[i] = cur
            end
